...
async function getIssue(projectId, repositoryId, issueId) {
    try {
        ...
        const response = await axiosInstance.get(`${projectId}/_apis/wit/workitems/${issueId}?api-version=7.1`);
...
async function addIssueComment(projectId, repositoryId, issueId, comment) {
    try {
        const axiosInstance = axios.create({
            baseURL: ORG_URL,
            headers: {
        ...
        const response = await axiosInstance.post(`${projectId}/_apis/wit/workitems/${issueId}/comments?api-version=7.1`, {
            text: comment,
        });
        return response.data;
    }
